...
export default function AddFormulaMilk() {
  ...
  const handleSubmit = async () => {
    ...
    if (form.id) {
      await db
        .update(formulaMilkTable)
        .set(form)
        .where(eq(formulaMilkTable.id, form.id));
    } else {
    ...
    <Form gap="$3" onSubmit={handleSubmit} p="$4" >
      ...
      <YStack>
        ...
        <Input
          ...
          onChangeText={(text) => updateForm("milkIntake", text ? parseInt(text): 0 )}
          placeholder="输入进食量 (ml)"
        />
      </YStack>

      <Form.Trigger asChild disabled={status !== "off"}>
        <Button  theme="accent" icon={status === "submitting" ? () => <Spinner /> : undefined}>
          {form.id ? "修改记录" : "添加记录"}
        </Button>
      </Form.Trigger>
    </Form>
